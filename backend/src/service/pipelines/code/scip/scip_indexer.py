...
from src.service.constants import EXTENSION_TO_SCIP, SKIP_DIRS

if TYPE_CHECKING:
    from src.models.code_schema import ScipParseResult
...
def file_path_has_ignore_dir_segment(file_path: Path, index_root: Path) -> bool:
    ...
    """
    ignore_dirs = {d.strip().lower() for d in SKIP_DIRS if d.strip()}
    if not ignore_dirs:
        return False
    try:
...
def is_scip_available(lang: str) -> bool:
    ...
    for ext, (mapped_lang, binary, _, docker_image) in EXTENSION_TO_SCIP.items():
        if mapped_lang == lang:
...
def detect_project_lang(path: Path, scip_languages: List[str]) -> Optional[str]:
    ...
    for ext, (lang, _, _, _) in EXTENSION_TO_SCIP.items():
        ...
        counts[lang] = counts.get(lang, 0) + sum(
            ...
            if not file_path_has_ignore_dir_segment(Path(_), path)
        )
    ...
    def run(self, project_path: Path, lang: str, output_dir: Path) -> Optional[Path]:
        ...
        if binary:
            cmd = self._build_command(
                lang, binary, project_path, output_file, scratch_dir=output_dir
            )
            if not cmd:
                logger.warning(f"No SCIP command template defined for language: {lang}")
                return None

            logger.info(f"Running local SCIP indexer: {' '.join(str(c) for c in cmd)}")
            try:
                ...
                if result.returncode == 0 and output_file.exists():
                    logger.info(f"SCIP index written to {output_file}")
                    return output_file
                logger.warning(
                    f"Local SCIP indexer failed (code {result.returncode}). stderr: {result.stderr[:500]}"
                )
            except Exception as e:
                logger.warning(f"Local SCIP indexer failed: {e}")

        # Fallback to Docker
        if docker_image and shutil.which("docker"):
            logger.info(f"Attempting SCIP indexing via Docker ({docker_image})...")
            try:
                # 1. Pre-run: for Go we often need 'go mod tidy'
                if lang == "go":
                    logger.info("Running 'go mod tidy' inside container first...")
                    subprocess.run(
                        [
                            "docker",
                            "run",
                ...
                if lang == "go" and not binary:
                    ...
                    internal_cmd = [
                        "bash",
                        "-c",
                        "dart pub global activate scip_dart && dart pub get && dart pub global run scip_dart ./",
                    ]
                elif lang in ("cpp", "c"):
                ...
                if result.returncode == 0:
                    if output_file.exists():
                        logger.info(f"SCIP index written to {output_file} via Docker")
                        return output_file
                    ...
                    if cwd_index.exists():
                        ...
                        logger.info(
                            f"SCIP index written to {output_file} via Docker (moved from project root)"
                        )
                        return output_file
                logger.error(
                    f"Docker SCIP indexing failed (code {result.returncode}). stderr: {result.stderr[:500]}"
                )
            except Exception as e:
                logger.error(f"Docker SCIP indexing failed: {e}")

        if not binary:
            logger.warning(
                f"SCIP indexer for '{lang}' not found locally or in Docker. Install with: {install_hint}"
            )
        return None
...
class ScipIndexParser:
    ...
    def parse(self, index_scip_path: Path, project_path: Path) -> "ScipParseResult":
        try:
            from . import scip_pb2  # type: ignore
        except Exception as e:
            logger.error(f"Failed to import scip2: {e}")
            return {}
        ...
        try:
            ...
            logger.error(f"Failed to parse SCIP index at {index_scip_path}: {e}")
            return {}

        from collections import defaultdict

        from src.models.code_schema import SymbolInfo

        symbol_def_table = defaultdict(SymbolInfo)
        rust_impls = defaultdict(set)
        ...
        for doc in index.documents:
            for occ in doc.occurrences:
                ...
                if role & 1:  # Definition
                    symbol_def_table[occ.symbol].file_path = doc.relative_path
                    symbol_def_table[occ.symbol].line = (
                        occ.range[0] + 1 if occ.range else 0
                    )
                    symbol_def_table[occ.symbol].span = self._parse_scip_range(
                        occ.range
                    )
                ...
                if impl_match:
                    ...
                    rust_impls[struct_name].add(trait_name)

        for doc in index.documents:
            for sym_info in doc.symbols:
                if sym_info.symbol in symbol_def_table:
                    symbol_def_table[sym_info.symbol].name = sym_info.display_name
                    symbol_def_table[sym_info.symbol].doc = "\n".join(
                        sym_info.documentation
                    )
                    symbol_def_table[sym_info.symbol].kind = sym_info.kind
                    symbol_def_table[sym_info.symbol].bases = [
                        rel.symbol
        ...
        files_data = {}
        doc_source_lines = defaultdict(list)
        for doc in index.documents:
            src_path = project_path / doc.relative_path
            try:
                doc_source_lines[doc.relative_path] = src_path.read_text(
        ...
        for sym, info in symbol_def_table.items():
            if info.kind == 0:
                ...
                line_num = info.line
                src_lines = doc_source_lines[file_path]
            ...
            if name in rust_impls and info.kind in (7, 18, 20, 49, 53, 54):
                info.bases = list(set(info.bases + list(rust_impls[name])))

        for doc in index.documents:
            rel_path = doc.relative_path
            abs_path = str((project_path / rel_path).resolve())
            source_lines = doc_source_lines[rel_path]
            ...
            file_data = ProcessedCode()
            file_data.path = abs_path
            file_data.language = self._lang_from_path(rel_path)
                ...
                if role & 1:  # Definition
                    ...
                    if kind == 0:
                        ...
                        if sym.endswith("().") and "#" in sym:
                            ...
                            # Check source to distinguish class/interface/trait
                            src = doc_source_lines[rel_path]
                            src_line = (
                                src[line - 1].strip().lower()
                                if 0 < line <= len(src)
                                else ""
                            )
                            if src_line.startswith("interface "):
                                ...
                                kind = 53
                            elif sym.startswith("scip-go") or sym.startswith(
                                "rust-analyzer"
                            ):
                                kind = 49
                            else:
                    ...
                    name = self._name_from_symbol(sym)
                    args, return_type = self._parse_signature(defn.name, kind)
                    ...
                    node = FunctionDef(
                        name=name,
                        kind=kind_str,
                        span=defn.span or Span(start_line=line, end_line=line),
                        doc_comment=defn.doc,
                        is_dependency=False,
                        args=args,
                    ...
                    if kind in (26, 17):
                        ...
                        node.bases = [self._name_from_symbol(b) for b in defn.bases]
                    ...
                    src_line = (
                        source_lines[ref_line_idx]
                        if ref_line_idx < len(source_lines)
                        else ""
                    )
                    char_after = (
                        src_line[col_end : col_end + 1]
                    ...
                    ref_span = self._parse_scip_range(occ.range)
                    caller_sym = self._find_enclosing_definition(
                        line, definition_symbols_in_doc
                    )

                    edge = FunctionCalls(
                        caller=sym, calling=self._name_from_symbol(sym)
                    )
                    edge.calle_file = (
                        str((project_path / callee_info.file_path).resolve())
                        if callee_info.file_path
                        else ""
                    )
                    edge.span = ref_span
                    edge.caller_file = abs_path
                    edge.ref_line = line

                    if caller_sym:
                        edge.caller = caller_sym
                        edge.typeCall = "Function-Level"
                        file_data.functionCalls.append(edge)
                    else:
                        edge.caller = ""
                        edge.typeCall = "Module-level"
                        file_data.functionCalls.append(edge)
    ...
    def _parse_scip_range(self, r: Any) -> Any:
        from src.models.code_schema import Span

        r_list = list(r)
        if len(r_list) == 4:
            return Span(
                start_line=r_list[0] + 1,
                start_column=r_list[1] + 1,
                end_line=r_list[2] + 1,
                end_column=r_list[3] + 1,
            )
        elif len(r_list) == 3:
            return Span(
                start_line=r_list[0] + 1,
    ...
    def _parse_signature(
        self, display_name: str, kind: int
    ...
    def _find_enclosing_definition(
        ...
        for occ in definition_occurrences:
            ...
            if not er:
                continue
            enc_start, enc_end = (
                (er[0] + 1, er[2] + 1) if len(er) == 4 else (er[0] + 1, er[0] + 1)
            )
