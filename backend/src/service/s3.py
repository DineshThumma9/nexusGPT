...
class S3Service:
    ...
    async def upload_file_to_s3(self, file: UploadFile, kb_id: str) -> str:
        """
        ...
        try:
            self.client.upload_fileobj(file.file, self.bucket, s3_key)
    ...
    def download_s3_to_tempfile(self, s3_key: str) -> str:
        """
        ...
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
                self.client.download_fileobj(self.bucket, s3_key, tmp)
                tmp_path = tmp.name
    ...
    def delete_prefix_from_s3(self, kb_id: str) -> None:
        """
