    ...
    for name in [COLLECTION_CODE, COLLECTION]:
        if not client.collection_exists(name):
            if name == COLLECTION:
                ...
                # 1. Create the tenant index for your Knowledge Base ID
                client.create_payload_index(
                    collection_name=name,
                    field_name="metadata.kb_id",
                    field_schema=models.KeywordIndexParams(
                        type=models.PayloadSchemaType.KEYWORD,
                        is_tenant=True,  # Valid: Tells Qdrant to partition data by this key
                    ),
                )

                # 2. Create a standard keyword index for the file path
                client.create_payload_index(
                    collection_name=name,
