...
┌──────────────────────────────▼──────────────────────────────────────┐
│                        Caddy (Reverse Proxy)                        │
│   Auto HTTPS (Let's Encrypt)  │  TLS termination  │  Gzip/Brotli   │
│   /api/* → FastAPI :8000      │  /* → Vite/static :5173            │
│   SSE streaming passthrough   │  WebSocket support                 │
└───────────────┬───────────────────────────┬────────────────────────┘
                │ HTTP (internal Docker net) │
┌───────────────▼──────────┐  ┌─────────────▼───────────────────────┐
│   React Frontend (Vite)  │  │      FastAPI Backend (Python)        │
│  Chakra UI │ Zustand     │  │ LangChain / LangGraph Agent          │
│  Axios     │ Shiki       │  │ Pydantic │ SlowAPI │ httpx │ Sentry  │
└──────────────────────────┘  │ SQLModel / SQLAlchemy │ LangSmith   │
                              └──────┬──────────┬──────────┬─────────┘
                                     │          │          │
                        ┌────────────┼──────────┼──────────┼──────────┐
                        │            │          │          │          │
                    ┌───▼──┐  ┌─────▼───┐  ┌───▼──┐  ┌───▼──┐  ┌───▼───┐
                    │Redis │  │Postgres │  │Celery│  │Qdrant│  │Neo4j  │
                    │broker│  │(Primary)│  │Worker│  │(Vec) │  │(Graph)│
                    └──────┘  └─────────┘  └──────┘  └──────┘  └───────┘
                                                           │
                                              voyage-code-3 / nomic-embed
                                                    │
                                             ┌──────▼──────┐
                                             │   AWS S3    │
                                             │ (Blob store)│
                                             └─────────────┘
                                           │                │
                              voyage-code-3 embeddings   Tree-sitter AST
                              nomic-embed (docs)         language pack
                                    │
                             ┌──────▼──────┐
                             │   AWS S3    │
                             │ (Blob store)│
...
All services orchestrated via **Docker Compose**, deployed on **AWS EC2**.
